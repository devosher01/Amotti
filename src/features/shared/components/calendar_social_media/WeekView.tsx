                  ...
                  <Box
                    ...
                    onDragOver={e => {
                      ...
                      if (draggedPost) {
                        ...
                        // Permitir el drop si es un target válido
                        const isValid = isValidDropTarget(timeSlot, day);
                        e.dataTransfer.dropEffect = isValid ? 'move' : 'none';
                        // Solo log ocasionalmente para evitar spam
                        if (Math.random() < 0.01) {
                          console.log('🔄 DragOver:', { isValid, timeSlot: timeSlot.toISOString() });
                        }
                      }
                    }}
                    onDragEnter={e => {
                    ...
                    onDrop={e => {
                      e.preventDefault();
                      e.stopPropagation();
                      console.log('📦 WeekView onDrop ejecutado:', {
                      ...
                      if (draggedPost && isValidDropTarget(timeSlot, day)) {
                        console.log('✅ Ejecutando onDrop callback');
                        onDrop(timeSlot, day);
                      } else {
                        console.log('❌ No se ejecuta onDrop - condiciones no cumplidas');
                      }
                    }}
                  >
                    {/* Container de posts con espaciado perfecto */}
                    <Box sx={{ 
                      padding: '6px', // Reducido para más espacio
                      height: '100%', 
                      display: 'flex', 
                      flexDirection: 'column', 
                      justifyContent: 'flex-start',
                      overflow: 'hidden',
                      gap: '3px', // Reducido para más espacio
                      minWidth: 0,
                    }}>
                      ...
                      {/* Indicador "más posts" refinado - solo si hay más de 2 */}
                      {slotPosts.length > 2 && (
                        <Box 
                          onClick={() => {
